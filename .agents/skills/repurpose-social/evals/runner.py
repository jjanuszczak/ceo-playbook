#!/usr/bin/env python3
"""Run configured checks for the Repurpose Social skill."""
...
def main() -> int:
    ...
    for step in config["pipeline"]["steps"]:
        completed = subprocess.run([sys.executable, str(base_dir / step["script"])], capture_output=True, text=True)
        status = "PASS" if completed.returncode == 0 else "FAIL"
        try:
            details = json.loads(completed.stdout.strip())
        except json.JSONDecodeError:
            details = {"message": completed.stdout.strip() or completed.stderr.strip()}
